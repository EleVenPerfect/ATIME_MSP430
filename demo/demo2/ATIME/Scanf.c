 ...
 *
 * 文件功能描述：
 *          MSP430F16x,MSP430F14X系列单片机scanf函数移植
 *      说明：
 *          该程序完成scanf函数的移植，使用后即可正常使用scanf函数
 *          从超级终端获取输入信息
 *          使用时只需把Scanf.c添加到工程中，然后在要调用scanf函数
 *          的源程序文件中加入对stdio.h的包含；然后就可以使用scanf
 *          函数从超级终端获取数据了。
 *
 * 创建标识：   刘中原20110730
 *
